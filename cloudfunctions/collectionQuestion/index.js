...
exports.main = async (event, context) => {
  ...
  if (answerStatus != false) {
    ...
    await updateAnswerStatusIndexQuestion(event.questionId, event.isCollection)
  } else {
    await addAnswerStatusIndexQuestion(event.questionId, event.isCollection)
  }
  return "sucess"
}
...
async function updateAnswerStatusIndexQuestion(questionId, isCollection) {
  let answerStatus = await db.collection('UserAnswerStatus').where({
    "questionId": questionId,
    "studyUserId": wxContext.OPENID
  }).update({
...
async function addAnswerStatusIndexQuestion(questionId, isCollection) {
  let answerStatus = {
